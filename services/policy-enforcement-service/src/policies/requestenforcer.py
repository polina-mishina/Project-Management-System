...
class RequestEnforcer:
    ...
    def __is_request_in_whitelist(self, request: Request) -> tuple[bool, str | None]:
    ...
    def __extract_token_data(self, request: Request) -> dict | None:
        try:
            if 'authorization' in request.headers:
                ...
                return jwt.decode(
                    token, self.jwt_secret, algorithms=["HS256"], audience=["fastapi-users:auth"]
                )
        except:
            return None
        return None

    def __check_by_policy(self, request: Request) -> tuple[bool, str | None]:
